...
::: incremental
-   "The null hypothesis states that there is no effect/no difference." [No!]{.fragment style="color:red;"}
...
```{r}
#| label: confint
#| fig-cap: "95% Confidence interval for an arbitrary parameter"
#| fig-align: left
#| echo: true
...
-   **Q:** What's the $p$-value for the values at the edge of the CI? [$p=0.05$ (for a 95%-CI)]{.fragment}
:::
:::::
...
## We can read off the $p$-value for any null value from the $p$-value function
...
How can we find the $p$-value of the hypothesis<br>$\operatorname{H}_0:$ IRR = $1$ from the plot? [$p \approx 0.25$]{.fragment}

[**Q:** How would you summarize the results?]{.fragment}
:::
:::::
...
: Hypothetical randomized trial data {#tbl-greenland12}
...
-   Does this mean that the study supports the conclusion that new treatment is equal to placebo in terms of AEs? [No!]{.fragment style="color:red;"}
:::
...
[Summary: Despite "nonsignificance" and power approximating 90% for $\operatorname{RR} = 2$, the results favor $\operatorname{RR} = 2$ over $\operatorname{RR} = 1$! [@greenland_nonsignificance_2012]]{.fragment}
...
*Implication:* No association for women with high alcohol consumption or effect differs between moderate and high consumption.
...
```{r}
#| label: schmidt_chao_meta_preparation
#| echo: false
#| output: false
...
mus <- log(seq(1.0, 1.45, length = 1000))

p_ed <- confMeta::p_edgington(
  c(log(1.20), log(1.20))
  , SEs = c(0.107002 , 0.0524792)
  , mu = log(seq(1.0, 1.45, length = 1000))
)
...
schmidt_meta$plot + 
  geom_line(data = meta_dat, aes(x = values, y = p_two), linewidth = 1.5)
...
-   Standard error can be extracted from published confidence intervals or from $p$-value if not provided.
...
## Software: R package [`parameters`](https://easystats.github.io/parameters/)

Allows the creation of $p$-value functions directly from models using the function [`p_function`](https://easystats.github.io/parameters/reference/p_function.html). Here is an example from a linear regression model:

```{r}
#| label: parameters_example
#| echo: true
#| fig-align: left
#| out-width: 6.6in
#| fig-asp: 0.706
...
::: incremental
-   The practice of dichotomizing results into "significant" and "not significant" is not informative [@gelman_difference_2006].
-   Focus on effect sizes and confidence/compatibility intervals.
-   $P$-value functions:
    -   are more informative than commonly used summaries of study results.
    -   require minimal training because they show familiar measures ($p$-values and confidence intervals).
:::

## Take-Home-Messages

::: incremental
-   Create $p$-value functions to summarize the available evidence and improve statistical reasoning:
    -   Values with high and low compatibility with the data.
    -   How informative was the study?
    -   To compare evidence from different studies
    -   Useful for meta-analyses [see @held_comparison_2025; and @rucker_beyond_2021].
